...
define(function (require) {
	...
	function init (name) {
		...
		var pools = _getLoadPools(name);	// 加载者集合

		if (_initPools[name] == undefined) {
			_initPools[name] = args || [];
			_applyLoaders(name, args);
		} else {
	...
	function load (name, func) {
		...
		if (_isFn(func)) {
			if (_initPools[name] == undefined) {
				_loadPools[name] = _getLoadPools(name);
				...
				_applyLoaders(name, _initPools[name]);
			}
		} else {
	 ...
	 * 获取加载者集合
	 */
	function _getLoadPools (name) {
		return _loadPools[name] || [];
	}


	// 响应时锁定，防止类似并发情况
	var _applyLock = false;

	/*
	 * 响应集合中的加载者
	 */
	function _applyLoaders(name, params) {
		if (_applyLock) return;
		_applyLock = true;

		var loaders = _getLoadPools(name);
		while(loaders.length) {
			_apply(loaders.shift(), params, function (e) {
	...
	function _call(name, params) {
		try {
			...
			if (data && _isFn(data.fn)) {
				// 执行事件并返回结果
				var result = _apply(data.fn, params, function (e) {
					if (_isFreed(e)) {
				...
				for(var user in temp) {
					_apply(temp[user], args, function (e) {
						if (_isFreed(e)) {
							...
							_log('error', 'hz.event.emit：订阅对象（'+ user + '）处理消息（'+ name + '）时内部错误：' + e);
						}
					});
				}
			} else {
