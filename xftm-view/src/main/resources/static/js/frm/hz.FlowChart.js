...
define(function  (repuire) {
	...
	function PlanChart(){
		...
		return {
		    ...
		    /*流程图初始化
		     * @param	myChart 	存放流程图的标签
		     * @param	data		节点数据
		     * @param	drag 		是否拖动节点(boolean)
		     * @param	rightMenu	禁止右键菜单
		     * 
		     * @param	tool			功能（自定义右键菜单）
		     * @param	tool.id 		元素id
			 * @param	tool.name 	功能名称
			 * @parma	tool.icon 	图标样式
			 * 
			 * @parma	fmtTip		提示框回掉函数
			 * @parma	callback	节点拖动的回掉函数
		     * */
			init:function(para){
				...
				para.fmtTip && (fmtTip = para.fmtTip) && this.moveOut();//移入节点
				return this.stepMap;
			},
			/*下拉列表 
			 * @param inputDt			显示的框; 
			...
			add:function(item,chartId,classN){
				...
				var flex = $("<div class='"+classN+"'><div class='list'><i></i></div></div>").appendTo(chartId);
			...
			/*鼠标抬起*/
			myMouseup: function(listEn, fal){
				var that = this,listUp;
				var targetList = listEn.parentNode;/*拖动的元素*/
				...
				document.onmouseup = function (e) {
	               ...
	               /*通过鼠标抬起时的坐标获取该坐标所在的元素*/
	                ...
	                if(listUp){
	                	...
	                	that.$class(listUpParent,'lineS')[0].appendChild(targetList)
	                	//添加相应数据
	                	if(fal){
	                		parentId = that.$class(listUpParent,'list')[0].id;
	                		...
	                		if(callback){
	                			if(suredStep){
	                				if(!(listUp.classList.contains('active'))){
		                				...
		                				that.stepMap[listUp.id].status = 'normal';
			                			var arrt = that.stepMapID(listData);
			                			...
			                			if(listEn.classList.contains("normal")){
			                				that.stepMap[listEn.id].status = 'waiting';
			                				listEn.classList.add('waiting');
			                				listEn.classList.remove('normal');
			                			};
			                		};
	                			};
		                		callback(listData);
			...
			moveOut: function(){
				//移入节点
				var myTip = this.myTips();
				var myChart = this.myChart;
				var that = this,tip;
				this.myChart.on("mouseenter", ".list", function(e){
					
					var thisId = $(this).attr("id");
					var data = that.stepMap[thisId];
					if (data) {
						tip = fmtTip && fmtTip(data) || data.tip;
						tip != '' && myTip.html(tip).show();
					} else {
						myTip.hide();
					}
				});
				that.mouseMovement(myChart, myTip);
			},
			/*鼠标移入移出
			*/
			mouseMovement: function(myChart, myTip){
				//移入
				myChart.on("mousemove", ".list", function (e) {
					var winHeight = $(window).height();
					var winWidth = $(window).width();

					var offsetY = myChart.offset().top;
					var offsetX = myChart.offset().left;

					var pageX = e.pageX - offsetX + 15;
					var pageY = e.pageY - offsetY + 15;

					var tipWidth = myTip.width();
					var tipHeith = myTip.height();

					if (e.clientY + tipHeith + 30 > winHeight) {
						pageY -= tipHeith+40;
					}
					...
					myTip.css({'left':pageX, 'top':pageY});
				});

				/*
				 * 鼠标移出事件
				 */
				myChart.on("mouseleave",".list", function () {
					myTip.hide().html('');
				});
			},
			/*获取父级数据的id
			 ...
			 * */
			$class: function(){
				var param = arguments,e;
				if(param.length > 1){
					e = param[0].getElementsByClassName(param[1]);
				}else{
					e = document.getElementsByClassName(param[0]);
				}
