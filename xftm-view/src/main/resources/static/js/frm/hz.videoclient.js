...
define(function (require) {
	...
	window.top.__VIDEOCLIENT_DEBUG = false; // 定义是否打印日志的开关，可以通过控制台来修改该值，这样便可以控制日志输出（默认不打印）
    ...
    function printLog (msg) {
    	if (window.top.__VIDEOCLIENT_DEBUG) {
    		console.log(msg);
    	}
    }
	 ...
	 */
	function _initTalk(){
		printLog('初始化 --> 对讲Service模块...');
		if(!talkback_config){
			printLog('未配置对讲服务信息,请前往 对讲-对讲服务页面进行配置');
			return;
		}
		var bodyJson = {
	...
	var winVideoData = {};		// 窗口视频数据（实时播放、视频回放的请求数据），格式：{'窗口索引': '数据'}
	...
	function readCameraInfo (winIndex, callback) {
		if (winIndex != null && winIndex != -1) {
			msgRequest('VIDEO025', {}, 'VIDEO026', function (msg) {
				var data = msg.body.returnValue;
				if (data) {
					if (typeof data == 'string') {
						data = JSON.parse(data);
					}
				}
//				callback({'index': data.index, 'cameraId': data.cameraID, 'cameraName': '', 'cameraBrand': ''});
				callback(winVideoData[data.index]);
			});
		} else {
			// TODO: 这里需要从记录的数据中获取
			callback(winVideoData[winIndex]);
		}
	}
	...
	function playVideo (reqBody, callback) {
		// 记录播放实时视频的数据
		winVideoData[reqBody.index] = reqBody;
	...
	function initSpeech () {
		...
		var playHandle = emptyFun;	// 开始后的处理函数
		var stopHandle = emptyFun;	// 停止后的处理函数
		var callHandle = emptyFun;	// 播报后的处理函数
		...
		function execute (delay, times, text) {
			if (status && text) {
				...
				msgRequest('SPEECH001', {'speechStr':text}, 'SPEECH001', callHandle);
				if (--times > 0) {
		...
		function stop () {
			...
			playHandle = emptyFun;	// 开始后的处理函数
			stopHandle = emptyFun;	// 停止后的处理函数
			callHandle = emptyFun;	// 播报后的处理函数
		}
		 ...
		 * @param methods 播报处理方法{'stop':'停止后的处理函数', 'play': '开始播报后的处理函数', '', 'callback':'发送播报指令后的回调'}
		 */
		speech.play = function (text, times, delay, methods) {
			stop();
			status = true;
			count = 0;

			// 处理缺省的回调
			if (typeof times == 'object') {methods = times; times = 1;}
			if (typeof delay == 'object') {methods = delay; delay = 3000;}
			if (typeof methods != 'object') {methods = {};}

			playHandle = methods.play;		// 开始后的处理函数
			stopHandle = methods.stop;		// 停止后的处理函数
			callHandle = methods.callback;	// 播报后的处理函数

			if (typeof playHandle != 'function') {playHandle = emptyFun;}
			if (typeof stopHandle != 'function') {stopHandle = emptyFun;}
			if (typeof callHandle != 'function') {callHandle = emptyFun;}
