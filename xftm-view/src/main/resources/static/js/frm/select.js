define(function(require) {
	...
	var selectcomponent = vue.extend({
		...
		watch:{
			...
			'sname':function(val){
				if(this.sname && this.sname.length>0){
	...
	var treeSelectComponent = vue.extend({
		...
		methods : {
			...
			load:function(){
				...
				if(me.sqlid){
					...
					me.datas = me.list;
					me.findName();
					me.tree = $.fn.zTree.init(ul,s,me.datas);	
					me.tree.expandNode(me.tree.getNodes()[0]);
				}
		...
		props : ['tip','sqlid','id','whereid','params','isleaf','sname','selected','search','list'],//isleaf为根节点是否能点
		template : '<div class="input icon">'
	...
	var checktreeComponent = vue.extend({
		...
		props : [ 'tip','sqlid','ids','whereid','params','firstcheck','sname','list'],
		template : '<div class="input icon">'
