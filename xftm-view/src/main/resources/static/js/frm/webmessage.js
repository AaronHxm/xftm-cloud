define(function(require){
	...
	window.top.__WEBMESSAGE_DEBUG = false; // 定义是否打印日志的开关，可以通过控制台来修改该值，这样便可以控制日志输出（默认不打印）
	/*
     * 打印日志
     * @param msg 消息
     */
    function printLog (msg) {
    	if (window.top.__WEBMESSAGE_DEBUG) {
    		console.log(msg);
    	}
    }
    ...
    function MyWebmessage () {
        ...
        this.init = function (url,success) {
        	...
        	this.websocket.on('onmessage', 'webmessage', function (event) {
                var data = event.data;
                printLog('webmessage.receive --> ' + JSON.stringify(data));
