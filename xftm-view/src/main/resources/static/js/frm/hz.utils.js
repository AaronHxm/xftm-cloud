...
define(function (require) {
	 ...
	 * 获取服务器时间
	 */
	function _getServiceTime(){
		var dateTime = null;
		_ajax(top.BURL + 'date/getDate',
				{args: null},
			function(data){
				if(!_isNull (data)){
					dateTime = _formatterStrDate(data.date + " " + data.time);
				}else{
					dateTime = new Date();
				}
			},function(data){},false);
...
//	};
	 ...
	 * 日志添加
	 */
	function _addLog(userName, desc, type){
		_ajax(top.BURL + 'log/putLog', {'args': JSON.stringify({
			'userName': userName,
			'desc': desc,
			'type': type
		})}, function(data){});
	...
	try {
		...
		utils.addLog = _addLog;
		return hz.utils = utils;
	} catch (e) {
